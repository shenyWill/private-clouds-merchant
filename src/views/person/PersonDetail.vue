    ...
    <div class="people-detail">
        <div class="show-people-detail" v-if="Object.keys(personDetail).length">
            <div class="people-detail-image">
                <img :src="((personDetail.personnelImgList && personDetail.personnelImgList[0]) ? (personDetail.url + personDetail.personnelImgList[0]) : (personDetail.personnelImgList[1] ? (personDetail.url + personDetail.personnelImgList[1]) : (personDetail.url + personDetail.personnelImgList[2])))" alt="">
        ...
        <div class="no-people-detail" v-if="!Object.keys(personDetail).length">
            <i class="iconfont icon-tubiaobeishanchu"></i>
            <p class="no-people-detail-content">该人员已被删除！</p>
        </div>
    </div>
</template>
...
.people-detail-content-key {
  font-size: 18px;
  font-weight: bold;
}
.people-detail-equipment {
  position: relative;
  .people-detail-content-key {
    position: absolute;
  }
}
.people-detail-content-equipment {
  margin-left: 97px;
  display: inline-block;
  line-height: 30px;
}
...
.no-people-detail {
    font-size: 23px;
    color: #1296db;
    .icon-tubiaobeishanchu {
        font-size: 120px;
    }
    .no-people-detail-content {
        text-indent: 15px;
        font-weight: bold;
    }
}
